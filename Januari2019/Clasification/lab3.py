...
if __name__ == '__main__':
    ...
    zborovi = list(getwords(recenica))
    kategorii = cl.categories()

    #print(zborovi)
    #print(list(kategorii))

    for zbor in zborovi:
        for kat in kategorii:
            tezinska = cl.weightedprob(zbor,kat)
            prob = cl.getFeaturePerCategoryProbability(zbor,kat)
            print(zbor,kat, round(prob,4), round(tezinska,4))

    katerorija, verojatnost = cl.classifyDocument(recenica)

    x = round(math.log(verojatnost,2),4)
